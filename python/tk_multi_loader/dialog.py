...
class AppDialog(QtGui.QWidget):
    ...
    def _load_entity_presets(self):
        ...
        for setting_dict in app.get_setting("entities"):
            ...
            if type_hierarchy:
                ...
                action_ia = QtGui.QAction("Import All LookDev and Animations for {0}".format(entity['name']), view)
                action_ia.setToolTip(
                    "<nobr>ZS IMPORT ALL LOOKDEV AND ANIMATION FOR {0}</nobr><br><br>"
                    "This action imports all LookDev files and animation cache mounts the shot completely<br><br>".format(entity['name'])
                )
                action_ia.hovered.connect(lambda: action_hovered(action_ia))
                action_ia.triggered.connect(self._import_lookdev_animations)
                view.addAction(action_ia)
                self._dynamic_widgets.append(action_ia)
    ...
    def _import_lookdev_animations(self):
        import json
        ...
        try:
            ...
            for asset in data:
                ...
                for load in data[asset]:
                    filenameLook = load['reference_file'].split('/')
                    filenameLook = filenameLook[-1:][0]
                    print('IMPORT LOOKDEV -> {0}'.format(filenameLook))
                    getFilename = filenameLook.replace('.ma', '')
                    getFilename = getFilename.replace('.', '_')
                    namespace = getFilename
                    groupName = '{}RN'.format(getFilename)
                    pm.system.createReference(load['reference_file'], mergeNamespacesOnClash=False, namespace=namespace, groupName=groupName, groupReference=True)
                    cmds.referenceQuery(load['reference_file'], referenceNode=True)
                    selected = cmds.ls(selection=True)[0]
                    target = cmds.pickWalk(selected, direction='down')[0]
                    target = target.split(':')[0]
                    target = '{0}:{1}'.format(target, load['master_group'])
                    cmds.AbcImport(load["alembic_path"], mode='import', connect=target)
